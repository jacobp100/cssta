...
const defaultDynamicComponent = withEnhancers([
...
it("uses fallback for variable if not defined within scope", () =>
  runTest({
...
it("transitions values using custom properties", () =>
  runTest({
    ruleTuples: [
      {
        validate: () => true,
