...
const getCssOutput = state => {
  ...
  return {
    ...
    read: () => fs.readFileSync(cssFilename, "utf-8"),
...
const removeContentBetween = (startToken, endToken, text) => {
  const startIndex = text.indexOf(startToken);
  const endIndex = text.lastIndexOf(endToken);
  return startIndex !== -1 && endIndex !== -1
    ? removeRange(startIndex + startToken.length, endIndex - 1, text)
    : text;
};
...
module.exports = {
  enter: (path, state) => {
    ...
    currentWebCss = removeContentBetween(
      commentStartMarker,
      commentEndMarker,
      currentWebCss
    );
